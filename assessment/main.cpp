...
int main() {
 ...
 /* test */
  strcpy(route, "W,W,W,W,W,W,SW,SE,SE");
  cout << "Starting at Edgware Road (Bakerloo Line) and taking the steps:" << endl;
  cout << route << endl;
  int result = validate_route(map, height, width, "Edgware Road (Bakerloo Line)", route, destination);
  if (result >= 0)
